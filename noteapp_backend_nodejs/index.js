...
app.use(express.static(path.resolve(__dirname, '../noteapp_react/build')))
app.use(
    session({
        resave: false,
...
app.get('/', async (req, res) => {
    await res.sendFile(path.resolve(__dirname, '../noteapp_react/build', 'index.html'))
})
...
app.post('/addImage', upload.single('thefile'), async (req, res) => {
    ...
    try {
        ...
        let result
        result = cloud.upload()
        if(result.url) res.status(200).json({"result": image})
    } catch(err) {
        console.log(err)
        res.status(500).json(err)
    }
